...
try:
    ...
    from climate_sim.utils.math_core import area_weighted_mean
except ModuleNotFoundError:
    from constants import ATMOSPHERE_MASS, EARTH_SURFACE_AREA_M2, GAS_CONSTANT_J_KG_K
    from math_core import area_weighted_mean

if TYPE_CHECKING:  # pragma: no cover - import used only for type checking
    from climate_sim.modeling.diffusion import DiffusionOperator
...
def _grid_latitude_and_diffusion(shape: tuple[int, int]) -> tuple[np.ndarray, "DiffusionOperator"]:
    ...
    try:
        from climate_sim.modeling.diffusion import DiffusionConfig, create_diffusion_operator
    except ModuleNotFoundError:  # pragma: no cover - support running module as a script
        from modeling.diffusion import DiffusionConfig, create_diffusion_operator
