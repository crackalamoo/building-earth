...
def main() -> None:
    ...
    surface_area_mean = float(area_weighted_mean(surface_cycle.mean(axis=0), cell_areas))
    ...
    if atmosphere_cycle is not None:
        atmosphere_area_mean = float(area_weighted_mean(atmosphere_cycle.mean(axis=0), cell_areas))
        print(
            "Atmosphere layer: "
    ...
    if wind_u is not None and wind_v is not None and wind_speed is not None and not args.headless:
        ...
        fig_wind, ax_wind = plt.subplots(figsize=(12, 6), subplot_kw=dict(projection=projection))
        ax_wind.set_global()
        ...
        ax_wind.add_feature(cfeature.LAND, facecolor="#f5f5f5", edgecolor="none", zorder=0)
        ...
        wind_speed_10m_sorted = (
            wind_speed_10m[:, :, lon_sort_idx] if wind_speed_10m is not None else None
        )
        ...
        wind_speed_geo_sorted = (
            wind_speed_geo[:, :, lon_sort_idx] if wind_speed_geo is not None else None
        )
        ...
        def _draw_streamplot() -> None:
            ...
            ax_wind.set_title(f"Wind ({current_state['level']}) – {month_names[idx]}")
    ...
    if humidity_q_cycle is not None and not args.headless:
        ...
        for month_idx in range(12):
            ...
            rh = specific_humidity_to_relative_humidity(
                q, temp_K, itcz_rad=itcz_rad, lat2d=lat2d, lon2d=lon2d
            )
        ...
        ax_humidity.add_feature(cfeature.LAND, facecolor="#f5f5f5", edgecolor="none", zorder=0)
        ...
        def _update_humidity_plot() -> None:
            ...
            norm_humidity, colorbar_label_humidity = get_norm_and_label(
                current_state_humidity["type"]
            )
        ...
        def format_humidity_message(
            lon: float, lat: float, data: dict, lon_idx: int, lat_idx: int
        ) -> str:
        ...
        def _update_humidity_plot() -> None:
            original_update()
            humidity_hover_data["field"] = humidity_data[current_state_humidity["type"]][
                current_state_humidity["month"]
            ]
    ...
    if not args.headless:
        ...
        if has_soil_moisture or has_vegetation or has_snow_ice or has_albedo:
            ...
            ax_surface.add_feature(cfeature.LAND, facecolor="#f5f5f5", edgecolor="none", zorder=0)
            ...
            if has_vegetation:
                surface_props_data["Vegetation Fraction"] = vegetation_fraction_cycle[
                    :, :, lon_sort_idx
                ]
                surface_props_is_monthly["Vegetation Fraction"] = True
            if has_snow_ice:
                surface_props_data["Snow/Ice Fraction"] = snow_ice_fraction_cycle[
                    :, :, lon_sort_idx
                ]
            ...
            norm_surface, colorbar_label_surface = get_surface_norm_and_label(
                current_state_surface["type"]
            )
            ...
            def _update_surface_plot() -> None:
                data = get_surface_data(
                    current_state_surface["type"], current_state_surface["month"]
                )
                norm_surface, colorbar_label_surface = get_surface_norm_and_label(
                    current_state_surface["type"]
                )
            ...
            surface_hover_data = {
                "field": get_surface_data(
                    current_state_surface["type"], current_state_surface["month"]
                ),
                "type": current_state_surface["type"],
            }

            def format_surface_message(
                lon: float, lat: float, data: dict, lon_idx: int, lat_idx: int
            ) -> str:
                value = data["field"][lat_idx, lon_idx]
                var_type = data["type"]
            ...
            def _update_surface_plot() -> None:
                ...
                surface_hover_data["field"] = get_surface_data(
                    current_state_surface["type"], current_state_surface["month"]
                )
    ...
    if ocean_u is not None and ocean_v is not None and not args.headless:
        ...
        ax_ocean.add_feature(cfeature.LAND, facecolor="#f5f5f5", edgecolor="none", zorder=0)
        ...
        ocean_speed = np.where(
            np.isnan(ocean_u_sorted) | np.isnan(ocean_v_sorted), np.nan, ocean_speed
        )
    ...
    for idx in range(12):
        ...
        if wind_speed_10m is not None:
            ...
            print(
                f"  Wind speed 10 m mean (land/ocean) [m/s]: {wind10_land:.2f} / {wind10_ocean:.2f}"
            )
        else:
            reason = (
                "bulk exchange disabled"
    ...
    print(
        f"  Surface mean (land/ocean) [°C]: {surface_land_annual:.2f} / {surface_ocean_annual:.2f}"
    )
    ...
    if atmosphere_cycle is not None:
        ...
        print(
            f"  Atmosphere mean (land/ocean) [°C]: {atm_land_annual:.2f} / {atm_ocean_annual:.2f}"
        )
    else:
    ...
    if wind_speed is not None:
        ...
        print(
            f"  Wind speed mean (land/ocean) [m/s]: {wind_land_annual:.2f} / {wind_ocean_annual:.2f}"
        )
    else:
        reason = (
    ...
    if wind_speed_10m is not None:
        ...
        print(
            f"  Wind speed 10 m mean (land/ocean) [m/s]: {wind10_land_annual:.2f} / {wind10_ocean_annual:.2f}"
        )
    else:
        reason = (
            "bulk exchange disabled" if not sensible_heat_config.enabled else "winds unavailable"
    ...
    if not args.headless:
        ...
        if data_dir_value:
            ...
            if "Surface" in plot_layer_cycles:
                _save_cycle(
                    "Surface", plot_layer_cycles["Surface"], "surface_temperature_cycle.gif"
                )
            if "Atmosphere (2 m)" in plot_layer_cycles:
                _save_cycle(
                    "Two-meter",
                    plot_layer_cycles["Atmosphere (2 m)"],
                    "two_meter_temperature_cycle.gif",
                )
            if "Atmosphere" in plot_layer_cycles:
                _save_cycle(
                    "Atmosphere",
                    plot_layer_cycles["Atmosphere"],
                    "atmosphere_temperature_cycle.gif",
                )
